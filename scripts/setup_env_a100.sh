#!/usr/bin/env bash
# Source this file once per shell before running OPSD on an A100 80G server:
#   source scripts/setup_env_a100.sh 4
#
# Mirrors the tads/weasel/CROPI convention:
# - code checkout stays under /home/kms/dev
# - all heavy artifacts live under /group-volume
...
if [ -z "${OPSD_WORK:-}" ]; then
  export OPSD_WORK="${GROUP_VOLUME%/}/${OPSD_USER}/opsd"
fi
...
export OPSD_ACCELERATE_CONFIG="${OPSD_ACCELERATE_CONFIG:-$OPSD_REPO_ROOT/accelerate.a100.yaml}"
...
# Shared model mirror. download_models_a100.sh writes here unless overridden.
...
case "$OPSD_NUM_GPUS" in
  1|2|4|8) ;;
  *) echo "[setup_env_a100] WARN: unusual OPSD_NUM_GPUS=$OPSD_NUM_GPUS; expected 1,2,4,8" >&2 ;;
esac
...
export OPSD_CUDA_VISIBLE_DEVICES="${OPSD_CUDA_VISIBLE_DEVICES:-$(_opsd_gpu_list "$OPSD_NUM_GPUS")}"
...
opsd_activate() {
  if [ ! -f "$OPSD_VENV/bin/activate" ]; then
    ...
    echo "[opsd_activate] create it: bash scripts/install_a100.sh" >&2
    return 1
  fi
...
_opsd_warn() {
  ...
  if [ ! -e "$path" ]; then
    if [ "$_opsd_missing" = "0" ]; then
      echo ""
      echo "------------------------------------------------------------------"
      echo "[setup_env_a100] WARNINGS: the following paths do not exist yet."
      echo "------------------------------------------------------------------"
    fi
...
_opsd_warn OPSD_VENV "$OPSD_VENV" "bash scripts/install_a100.sh"
_opsd_warn MODEL_PATH_QWEN3_17B "$MODEL_PATH_QWEN3_17B" "bash scripts/download_models_a100.sh 1.7b"

if [ "$_opsd_missing" -gt 0 ]; then
  echo "------------------------------------------------------------------"
  echo "[setup_env_a100] $_opsd_missing path(s) missing; env vars are still exported."
  echo "------------------------------------------------------------------"
else
  echo "[setup_env_a100] All required paths verified"
fi

echo ""
echo "OPSD A100 env loaded."
opsd_print_env

unset -f _opsd_warn
unset _opsd_missing _opsd_requested_gpus _opsd_detected_gpus
